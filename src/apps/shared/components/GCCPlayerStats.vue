  ...
  >
    ...
    <article v-if="!isLoading && stats">
      <section class="gcc-stats__profile">
        <div
          v-if="stats.initial && stats.initial.playerInfo"
          class="gcc-stats__profile-content"
        >
          <div class="gcc-stats__core-info">
            <div class="gcc-stats__avatar-wrapper">
              <div
                class="gcc-stats__avatar"
                :style="{
            ...
            <div class="gcc-stats__core-general-info">
              ...
              <transition-group class="gcc-stats__punishment-list" tag="ul">
                ...
                >
                  <img
                    :src="punishmentCardURL"
                    :alt="punishment.reason"
          ...
          <div class="gcc-stats__profile-social-medias">
            <a
              ...
              :key="social.icon"
              :href="social.url"
              class="gcc-stats__profile-social-media-buttom"
              :class="[
                `gcc-stats__profile-social-media-buttom--${social.name}`,
              ]"
              target="blank"
            >
      ...
      <section class="gcc-stats__profile-stats">
        <transition-group class="gcc-stats__profile-stats-list" tag="ul">
          <li
            v-for="stat in availableUserStats"
            :key="stat.name"
      ...
      <section
        v-if="historyMatchesNumbers.matches"
      ...
      >
        <div class="gcc-stats__matches-numbers">
          <span class="gcc-stats__match-number gcc-stats__match-number--win">
            {{ i18n.getMessage("playerStats__wonMatches") }}:
            <strong>{{ historyMatchesNumbers.wins }}</strong>
          </span>
          <span class="gcc-stats__match-number gcc-stats__match-number--total">
            {{ i18n.getMessage("playerStats__totalMatches") }}:
            {{ historyMatchesNumbers.matches }}</span
          >
          <span class="gcc-stats__match-number gcc-stats__match-number--loss">
            {{ i18n.getMessage("playerStats__lostMatches") }}:
            <strong>{{ historyMatchesNumbers.loss }}</strong>
          </span>
        </div>
        <div class="gcc-stats__matches-slider-wrapper">
          <vue-slider
            v-model="historyMatchesNumbers.wins"
            :interval="1"
            :marks="[]"
            :min="0"
        ...
        <div class="gcc-stats__matches-numbers">
          <span class="gcc-stats__match-number gcc-stats__match-number--win">
            {{
              (
                (historyMatchesNumbers.wins * 100) /
                historyMatchesNumbers.matches
              ).toFixed(1)
            }}%
          </span>
          <span class="gcc-stats__match-number gcc-stats__match-number--loss">
            {{
              (
                (historyMatchesNumbers.loss * 100) /
                historyMatchesNumbers.matches
              ).toFixed(1)
            }}%
          </span>
      ...
      >
        <div class="gcc-stats__maps-stats-header">
          {{ i18n.getMessage("playerStats__lastMatches") }}
        </div>
        <transition-group class="gcc-stats__maps-stats-list" tag="ul">
          <li
            v-for="mapStat in mapsStats"
            :key="mapStat.name"
            class="gcc-stats__map-stats-item"
            :class="{
          ...
          >
            ...
            <div
              v-if="mapStat.percentage"
              class="gcc-stats__map-stats-item-number-wrapper"
            >
              <div
                class="
              ...
              >
                ...
                >
                  {{ mapStat.wins }}
                  <p class="gcc-stats__map-stats-item-number--percentage">
                    {{ mapStat.percentage.wins }}
                  </p>
                </span>
              </div>
              <div
...
import { gcUrls, gcAssetsUrls } from "@/apps/shared/extras/gc/api/resources/urls";
...
import { GCMonthMatch } from "@/apps/shared/extras/gc/types/GCMonthMatch";
import { percentage } from "@/apps/shared/extras/gc/api/resources/constants/magicNumbers";
...
export default class GCCPlayerStats extends Vue {
  ...
  stats: Partial<{
    core: Partial<{
      social: Record<
        socialMedia,
        Partial<{ url?: string; name: string; icon: string }>
      >;
      statistics: Partial<{
        name: string;
        icon: string;
  ...
  get isLoading() {
    return (
      this.isLoadingInitialData && this.isLoadingHistory && this.isLoadingPlayer
    );
  }
  ...
  get userBackground() {
    ...
    return image
      ...
      : {
          "background-image": `url(${window.browser.runtime.getURL(
            "../../assets/logo_500.png"
          )})`,
  ...
  getCsgoMapImage(mapName: string) {
    const url = window.browser.runtime.getURL(
      `../../assets/csgo_maps/${mapName}.png`
    );
    return {
  ...
  get socialButtons() {
    return Object.values(this.stats.core?.social as any).filter(
  ...
  get availableUserStats() {
    ...
    const totalStatsNames = Object.values(this.totalStatsMap).map(
      (item) => item.name
    );

    stats.map((stat) => {
      const historyStat = this.stats.history?.stat.find(
        (currentHistoryStat) =>
          currentHistoryStat.stat?.toLowerCase() === stat.name?.toLowerCase()
      );
      if (historyStat) {
        stat.value = historyStat.value;
        const numberValue = parseInt(stat.value, 10);
        if (
          totalStatsNames.includes(stat.name as string) &&
          numberValue &&
          this.historyMatchesNumbers?.matches
        ) {
  ...
  receivePlayerPage(data: any) {
    ...
    socialKeys.forEach((socialKey) => {
      ...
      const selector =
        gcSelectors.playerPage.socialButtons[socialKey as socialMedia];
      if (social && selector) {
